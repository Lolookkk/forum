...
describe('Tests d\'intégration — API Utilisateur (/api/users)', () => {
  ...
  afterAll(async () => {
    await db.query('DELETE FROM users WHERE email = $1 OR email = $2 OR username = $3', [testUser.email, 'nouveau_email@example.com', 'nouveau_nom']);
    await db.end();
  });
  ...
  describe('POST /api/auth/login après modifications', () => {
    ...
    it('devrait échouer à la connexion avec l\'ancien email ou l\'ancien mot de passe', async () => {
      const loginResOldEmail = await request(app)
        .post('/api/auth/login')
        .send({
          email: 'integration_test@example.com',
          password: 'password123'
